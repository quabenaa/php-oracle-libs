...
class Oracle{
    ...
    */
    public function GetError(){
        return oci_error($this->errors_pool);
    }
    ...
    public function Connect($host='localhost/orcl', $user='', $pass='', $mode = OCI_DEFAULT, $type = ORA_CONNECTION_TYPE_DEFAULT){
        switch ($type) {
            case ORA_CONNECTION_TYPE_PERSISTENT: {
                $this->conn_handle = oci_pconnect($user, $pass, $host);
            }; break;
            case ORA_CONNECTION_TYPE_NEW: {
                $this->conn_handle = oci_new_connect($user, $pass, $host);
            }; break;
            default:
                $this->conn_handle = oci_connect($user, $pass, $host);
        }
    ...
    */
    public function Cursor($stored_proc, $params = false, $params_values = false, $cursor_v = false){
        if (!is_resource($this->conn_handle)) return false;
        if ($params) {
          if (is_array($params)) $parameters = implode(",", $params);
          $sql = "begin $stored_proc($parameters,:$cursor_v); end;";
          $stmt = oci_parse($this->conn_handle, $sql);
          foreach ($params_values as $key => $val) {
                // oci_bind_by_name($stid, $key, $val) does not work
                // because it binds each placeholder to the same location: $val
                // instead use the actual location of the data: $ba[$key]
                oci_bind_by_name($stmt, $key, $params_values[$key],$this->var_max_size,$this->GetBindingType($params_values[$key]));

                //echo $key.' - - - '.$params_values[$key].'<br/>';
            }
        } else {
          $sql = "begin $stored_proc(:$cursor_v); end;";
          $stmt = oci_parse($this->conn_handle, $sql);
        }
        $curs = oci_new_cursor($this->conn_handle);
        oci_bind_by_name($stmt, $cursor_v, $curs, -1, OCI_B_CURSOR);
        oci_execute($stmt);
        $this->errors_pool = $stmt;
        //execute cursor
        $this->execute_status = @oci_execute($curs,OCI_DEFAULT);
        $this->errors_pool = $curs;
        $this->FreeStatement($stmt);
        //Response Data
        $response['statement']=$this->execute_status ? $curs : false ;
        $response['status']=$params_values[':STATUS'];
        $response['message']=$params_values[':STATUS_MSG'];
        return $response;
    }
